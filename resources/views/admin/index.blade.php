@extends('admin.master') @section('content')
<div class="content-area py-1">
    <div class="container-fluid">
        <div class="row row-md">
            <div class="col-lg-3 col-md-6 col-xs-12">
                <div class="box box-block bg-white tile tile-1 mb-2">
                    <div class="t-icon right"><span class="bg-danger"></span><i class="ti-shopping-cart-full"></i></div>
                    <div class="t-content">
                        <h6 class="text-uppercase mb-1">Đơn Hàng Mới</h6>
                        <h1 class="mb-1" id="new-order"></h1>
                    </div>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-xs-12">
                <div class="box box-block bg-white tile tile-1 mb-2">
                    <div class="t-icon right"><span class="bg-success"></span><i class="fa fa-bars"></i></div>
                    <div class="t-content">
                        <h6 class="text-uppercase mb-1">Bài Đăng Góc Sách Mới</h6>
                        <h1 class="mb-1" id="new-post"></h1>
                    </div>
                </div>
            </div>
            {{-- {{ dd($books) }} --}}
            <div class="col-lg-3 col-md-6 col-xs-12">
                <div class="box box-block bg-white tile tile-1 mb-2">
                    <div class="t-icon right"><span class="bg-primary"></span><i class="fa fa-book"></i></div>
                    <div class="t-content">
                        <h6 class="text-uppercase mb-1">Sản Phẩm</h6>
                        <h1 class="mb-1">{{ $books->count() }}</h1>
                    </div>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-xs-12">
                <div class="box box-block bg-white tile tile-1 mb-2">
                    <div class="t-icon right"><span class="bg-warning"></span><i class="fa fa-users"></i></div>
                    <div class="t-content">
                        <h6 class="text-uppercase mb-1">Người dùng</h6>
                        <h1 class="mb-1">{{ $users->count() }}</h1>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
